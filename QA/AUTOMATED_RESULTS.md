...
- Live M4 timing gates pass: refresh p95 error 101 ms (≤200 ms), AppKit-host paint p95 73 ms (<250 ms) with zero render failures, and five-second cycle p95 error 103 ms (≤200 ms). Exact values, UTC start time, commit SHA, and worktree state are generated at `QA/latest-timing.json` by every QA run.
...
Dedicated QA and product-manager review are pending the clean exact `1.0.2` candidate.
