# MacMeter 0.1.2 requirement traceability
...
| QA-01 | 100% metric calculation/conversion decision-path coverage; overall line coverage ≥85% | Verified | `MetricDecisionPath` executable set-equality test covers all 33 declared semantic paths; source-only line gate in `Scripts/qa.sh` is 1,707/1,854 (92.07%). Swift's exported LLVM data provides no branch counters, so the explicit production-path recorder is the authoritative branch gate rather than a line-coverage proxy. |
...
| VER-01 | Start/remain 0.x; only owner command `pass` authorizes 1.0.0 | Verified | Xcode single version authority and script policy tests; current artifact `0.1.2 (1)` |
