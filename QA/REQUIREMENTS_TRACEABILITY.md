...
| DISP-02 | Compact shows every selection; all four use network/top and CPU-temperature-battery/bottom; Cycle rotates every 5 seconds | Verified automated | Compact/Cycle × all 16 combinations render matrix; semantic tests prove every selection appears exactly once and lock the all-four rows and visible strings; a persistent compact host resists a one-point width proposal for all 15 non-empty selections, stays ≤180 pt wide and ≤24 pt tall, and proves the four-metric case uses two lines; live five-cycle p95 gate |
...
| QA-01 | 100% metric calculation/conversion decision-path coverage; overall line coverage ≥85% | Verified | `MetricDecisionPath` executable set-equality test covers all 33 declared semantic paths; source-only line gate in `Scripts/qa.sh` is 1,569/1,646 (95.32%). Swift's exported LLVM data provides no branch counters, so the explicit production-path recorder is the authoritative branch gate rather than a line-coverage proxy. |
...
| PERF-01 | Idle CPU ≤1% average, p95 ≤3%; RSS ≤80 MiB; ≤5 MiB growth/24 h | Failed attempt; policy decision pending | The first definitive run crossed literal RSS from 74,320 KiB to 90,736 KiB after about 3.5 measurement hours and was stopped with raw-bound failure evidence. Diagnostics showed RSS was dominated by shared SwiftUI/AppKit residency (90.7 MiB RSS versus 32.4 MiB physical footprint and 27 MiB heap). Evidence v2 now records `/bin/ps` RSS and `proc_pid_rusage` physical footprint side by side while retaining RSS as the active gate. Both memory triples plus CPU/wall deltas are independently recomputed from the bound CSV. A new 30-minute warm-up + 24-hour run awaits the owner's memory-policy decision. |
