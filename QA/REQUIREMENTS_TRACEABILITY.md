...
| QA-01 | 100% metric calculation/conversion decision-path coverage; overall line coverage ≥85% | Verified | `MetricDecisionPath` executable set-equality test covers all 29 declared semantic paths; source-only line gate in `Scripts/qa.sh` is 1,440/1,518 (94.86%). Swift's exported LLVM data provides no branch counters, so the explicit production-path recorder is the authoritative branch gate rather than a line-coverage proxy. |
...
| QA-04 | Light/dark, increased text, constrained layout, all combinations/modes | Verified render automation; manual readability pending | Cartesian render matrix; every Cycle page ≤136 pt plus all-metric Compact ≤300 pt and Default ≤480 pt across small/large/accessibility text, without forced clipping; one persistent hosted Default label must expand by more than 100 pt when changing live from CPU-only to all four metrics; physical constrained-menu readability remains manual |
