...
| QA-01 | 100% metric calculation/conversion decision-path coverage; overall line coverage ≥85% | Verified | `MetricDecisionPath` executable set-equality test covers all 29 declared semantic paths; source-only line gate in `Scripts/qa.sh` is 1,391/1,476 (94.24%). Swift's exported LLVM data provides no branch counters, so the explicit production-path recorder is the authoritative branch gate rather than a line-coverage proxy. |
