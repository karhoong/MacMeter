...
| DISP-02 | Compact shows every selection; all four use one status-bar-safe row; Cycle rotates every 5 seconds | Verified automated | Compact/Cycle × all 16 combinations render matrix; semantic tests prove every selection appears exactly once and lock the all-four order and visible string; a persistent compact host resists a one-point width proposal for all 15 non-empty selections, stays ≤180 pt wide and ≤24 pt tall, and proves the four-metric case remains one line; live five-cycle p95 gate |
