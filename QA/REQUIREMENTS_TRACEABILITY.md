...
| PERF-01 | Idle CPU ≤1% average, p95 ≤3%; RSS ≤80 MiB; ≤5 MiB growth/24 h | Pending elapsed run | Harness uses cumulative `proc_pid_rusage` CPU nanoseconds over monotonic intervals and non-harmonic 59/61-second cadence. Raw CPU/wall deltas are filename/size/SHA-bound to JSON; elapsed time, the ≥30-minute boundary, and cadence pattern are independently enforced before recomputing count, duration, average, nearest-rank p95, RSS baseline/max/growth, and thresholds. Default 30-minute warm-up + 24-hour run is not complete. |
