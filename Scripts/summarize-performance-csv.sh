...
LC_ALL=C awk -F, -v values_file="$values_file" -v stats_file="$stats_file" -v allow_zero="$allow_zero" '
  ...
  NR == 1 {
    expected = "elapsed_seconds,rss_kib,cpu_interval_percent,interval_seconds,cpu_delta_ns,wall_delta_ns,phase"
    if ($0 != expected) invalid("unexpected header")
    next
  }

  {
    if (NF != 7) invalid("expected seven columns")
    if (!is_uint($1)) invalid("elapsed_seconds is not an unsigned integer")
    if (!is_uint($2)) invalid("rss_kib is not an unsigned integer")
    if (!is_number($3)) invalid("cpu_interval_percent is not a nonnegative number")
    if (!is_number($4)) invalid("interval_seconds is not a nonnegative number")
    if (!is_uint($5)) invalid("cpu_delta_ns is not an unsigned integer")
    if (!is_uint($6)) invalid("wall_delta_ns is not an unsigned integer")
    if ($7 != "warmup" && $7 != "boundary" && $7 != "measurement") invalid("unknown phase")

    if (rows > 0 && ($1 + 0) < previous_elapsed) invalid("elapsed time moved backwards")
    if ($7 == "measurement" && rows > 0 && ($1 + 0) <= previous_elapsed) invalid("measurement elapsed time did not advance")
    previous_elapsed = $1 + 0
    rows++

    if (($6 + 0) == 0) {
      if (($5 + 0) != 0 || $3 != "0.000" || $4 != "0.000") invalid("zero wall delta has inconsistent derived values")
    } else {
      expected_cpu = sprintf("%.3f", 100 * ($5 + 0) / ($6 + 0))
      expected_seconds = sprintf("%.3f", ($6 + 0) / 1000000000)
      if ($3 != expected_cpu) invalid("cpu_interval_percent does not match raw deltas")
      if ($4 != expected_seconds) invalid("interval_seconds does not match raw wall delta")
    }
    ...
    if (($6 + 0) <= 0) invalid("measurement wall delta must be positive")
    count++
    sum_cpu_ns += $5 + 0
    sum_wall_ns += $6 + 0
    if (($2 + 0) > maximum) maximum = $2 + 0
    printf "%.3f\n", 100 * ($5 + 0) / ($6 + 0) >> values_file
  }
